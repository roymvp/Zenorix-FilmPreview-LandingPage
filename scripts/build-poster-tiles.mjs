 ...
 *
 * Every source in `public/media/posters/` is a real licensed poster, and they
 * arrive at two different sizes (197x276 for the Netflix set, 350x525 for the
 * rest). Laying those out directly would give tiles of two aspect ratios, so each
 * one is covered into a single 2:3 tile at a fixed size and written as WebP: the
 * wall paints twelve tiles at once behind a scrim and the rails paint twenty
 * more, so per-file weight matters far more than pixel-perfect fidelity.
 *
 ...
 * pool in `lib/content/movies.ts` (rails).
 */
